...
def verify_execution_payload_bid_signature(
    ...
    return bls.Verify(builder.builder_pubkey, signing_root, signed_bid.signature)
```
...
*Note*: The function is modified to use the `index` field in the `AttestationData` to signal the
...
def verify_execution_payload_envelope_signature(
    ...
    message = signed_envelope.message
    signing_root = compute_signing_root(
        message, get_domain(state, DOMAIN_BEACON_BUILDER)
    )
    return bls.Verify(message.builder_pubkey, signing_root, signed_envelope.signature)
```
...
def process_execution_payload(
    ...
    assert envelope.builder_pubkey == committed_bid.builder_pubkey
