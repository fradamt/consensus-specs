...
- **Finality piggyback** confirms a lower-height justified checkpoint when the
  ...
  current vote's target, so a timeout vote may carry a lower-height piggyback.
...
An honest validator maintains a vote record for anti-slashing and for the R1/R2
decision:
...
- `voted_timeout_at: Set[Height]` - heights at which the validator has signed a
  timeout vote (`target == Checkpoint()`). This is paper `Delta.tau`.
...
- If `T != Checkpoint()` and `H not in voted_target_at`, set
  `voted_target_at[H] = T`.
- If `T == Checkpoint()`, add `H` to `voted_timeout_at`.
...
(timeout) vote with `target = Checkpoint()`, unless an E1 lock at that height
requires re-submitting the locked target. R2 votes drive the timeout-cert branch
of `processHeight` on `state.timeouts`. Like any vote, an R2 vote may carry a
lower-height finality piggyback when the vote record says the piggyback is safe.
...
the validator MUST fall back to casting another R1 with the locked target. Any
lower-height piggyback is still decided by the separate piggyback gate. See
[E1 avoidance](#e1-avoidance).
...
if current_height in voted_finality_at:
    ...
    # target = Checkpoint() at this height would self-slash via E1.
    attestation_data.target = voted_finality_at[current_height]
elif current_height in voted_target_at:
    # Already cast a non-empty target at current_height and not locked:
    # cast an R2 timeout.
    attestation_data.target = Checkpoint()
else:
    # First non-empty vote at current_height (R1 justify): vote for the
...
when it points to a lower height than the current vote and the vote record shows
that the validator previously voted for the same target at that lower height and
has not timed out there.
